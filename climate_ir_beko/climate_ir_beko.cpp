#include "climate_ir_beko.h"
#include "esphome/core/log.h"

namespace esphome {
namespace climate_ir_beko {

static const char *const TAG = "climate.climate_ir_beko";
...
void BekoIrClimate::transmit_state() {
  uint32_t remote_state = 0x8800000;

  ESP_LOGD(TAG, "climate_beko_ir mode_before_ code: 0x%02X", mode_before_);
  ...
  ESP_LOGD(TAG, "climate_beko_ir mode code: 0x%02X", this->mode);
...
bool BekoIrClimate::on_receive(remote_base::RemoteReceiveData data) {
  uint8_t nbits = 0;
  uint32_t remote_state = 0;
  ESP_LOGD(TAG, "In on_receive, data length %d", data.size());

  if (!data.expect_item(this->header_high_, this->header_low_)) {
    return false;
  }
  
  ESP_LOGD(TAG, "on_receive found expected headers");
...
void BekoIrClimate::transmit_(uint32_t value) {
  calc_checksum_(value);
  ESP_LOGD(TAG, "Sending climate_beko_ir code: 0x%02" PRIX32, value);
...
void BekoIrClimate::calc_checksum_(uint32_t &value) {
  uint32_t mask = 0xF;
  uint32_t sum = 0;
